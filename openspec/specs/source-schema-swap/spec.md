# source-schema-swap Specification

## Purpose
Discover a restructured or replacement IRIMSV source schema live, diff it
against the approved source contract, AI-re-map only the affected entities
(schema-only prompts, human-gated), and resume delivery on approval — the
symmetric counterpart to the target-side `schema-swap`. A source-swap never
writes to the source: it is a read-discovery + re-mapping operation.
## Requirements
