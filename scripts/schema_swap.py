"""Schema-swap CLI (generic engine, §0.4-0.5).

Preview, or apply, adopting a new target schema (or engine).
...
This folds into `sync-engine schema-swap` when the generic CLI lands (Sec 10).
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Preview or apply a target schema swap")
    ...
    parser.add_argument("--confirm", metavar="TARGET_DB",
                        help="type the target database name to APPLY the swap destructively")
